  ...
  _createClass(MarkerClusterGroup, [{
    ...
    value: function componentDidMount() {
      ...
      if (this.props.params) {
        this.props.params.enableDefaultStyle && (this.props.map._container.className += ' marker-cluster-styled');

        !this.props.params.disableDefaultAnimation && (this.props.map._container.className += ' marker-cluster-animated');
      }
    }
  }, {
    key: 'addMarkerClusterGroupToMap',
    value: function addMarkerClusterGroupToMap(markers) {
      var markerClusterGroup = _leaflet2.default.markerClusterGroup();

      markers.forEach(function (marker) {
        markerClusterGroup.addLayer(_leaflet2.default.marker([marker.lat, marker.lng]));
      });
